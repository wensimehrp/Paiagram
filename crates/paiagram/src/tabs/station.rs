...
impl super::Tab for StationTab {
    ...
    fn main_display(&mut self, app: &mut App, ui: &mut egui::Ui) {
        let Some(handle) = app.stations.get_handle(self.station) else {
            ui.label("???");
            return;
        };
...
//     let mut widths_seconds: Vec<(f32, i32)> = Vec::with_capacity(entry_bucket[current_h].len() +
// 2);     widths_seconds.push((ui.clip_rect().left(), 0));
