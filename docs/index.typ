...
general. So here is Paiagram, read it with me -- PAI-YAA-GRAM. You can use Paiagram to figure out
...
timetabling, though. However, if you do want something to be added into the program,
#link("https://github.com/WenSimEHRP/Paiagram/issues")[feel free to open a ticket!]

= Getting Started

You can use the #link(links.home)[web version] or the desktop version. The web version runs on
Chromium 113+. Latest versions of Firefox and Safari are also supported. Despite the fact that
Paiagram supports Linux and Windows, we do not provide a pre-compiled version just yet. You would,
unfortunately, have to compile the desktop version yourself.
...
- #link(links.oudia-second)[OuDiaSecond]: Japanese diagramming tool developed by diagram-mania.
  Successor of OuDia.
- #link(links.pyetrc)[pyETRC]: predecessor of qETRC.
- #link(links.qetrc)[qETRC] (#link(links.qetrc-docs)[documentation]): diagramming tool developed by
  x.e.p..
