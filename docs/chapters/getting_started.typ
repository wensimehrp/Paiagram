...
You can download Paiagram from #link("https://github.com/wensimerhp/paiagram/releases")[Github releases] or #link("https://wensimehrp.github.io/Paiagram")[try it online]. The online version's performance would be worse compared to native versions, but the difference is usually acceptable. You can also download the webpage and run it locally in your browser.
...
For performance sake, internally, the edges in Paiagram are _not_ directional, instead they are bi-directional. However, you can manually specify the edge's direction.

For performance sake, internally, there could be _only one_ edge betwee two nodes. However, you can specify extra nodes and connect from those nodes. Those extra nodes _would_ instead be of "waypoint" type.

The graph model allows easy manipulation of network, and allows for managing much larger networks. This also means that Paiagram won't have the _Nobori/Kudari_ settings in OuDiaSecond, or _Shangxing/Xiaxing_ properties in qETRC/pyETRC & friends.
...
Paiagram doesn't support directly exporting to PDF or printing, but you can export each page to Typst, and further render that to SVG/PNG/PDF. The spacing on diagram pages would be reserved. Exporting _directly_ to PDF, and exporting to LaTeX, docx, odt, etc. would never be supported.

== Vehicle-based model <wip>
...
=== Interdiagram Vehicle Links and Patches <wip>
